...
const PlanPaymentCompleted = () => {
  ...
  useEffect(() => {
    const fetchReceiptDetails = async () => {
      try {
        const response = await fetch(`http://localhost:8050/pass/stripe/${sessionId}`);
        if (!response.ok) {
        ...
        // Match the plan from the session data
        const selectedPlan = pricingPlans.find((plan) => plan.name === sessionData.planName);
        if (!selectedPlan) {
  ...
  const generatePdfReceipt = () => {
    ...
    // Receipt Details
    doc.setFontSize(12);
    doc.text(`Order ID: ${receiptDetails.orderId}`, 20, 40);
    doc.text(`Order Date: ${receiptDetails.orderDate}`, 20, 50);
    doc.text(`Payment Method: ${receiptDetails.paymentMethod}`, 20, 60);
...
export default PlanPaymentCompleted;
